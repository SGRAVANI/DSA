...
function minEnergy(a,n)
{
   if(n==0)
   {
    return 0
   }
   let left=Math.abs(a[n]-a[n-1])+minEnergy(a,n-1)
   let right; 
     if(n>1)
   {
    right=Math.abs(a[n]-a[n-2])+minEnergy(a,n-2)
   }
   else{
    right=Infinity
...
//let arr=[10 ,20, 30, 10]
let arr=[7, 4, 4 ,2, 6, 6, 3, 4]
console.log(minEnergy(arr,arr.length-1))
